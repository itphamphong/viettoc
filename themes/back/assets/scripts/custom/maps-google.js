var MapsGoogle = function() {
    ...
    var mapGeocoding = function() {
                ...
                GMaps.geocode({
                    ...
                    callback: function (results, status) {
                        if (status == 'OK') {
                            ...
                            $("#LangLog").val(latlng.lat() + "," + latlng.lng());
                            google.maps.event.addListener(marker, 'dragend', function (evt) {
                                $("#LangLog").val(evt.latLng.lat().toFixed(6) + ', ' + evt.latLng.lng().toFixed(6));
                            });
                        }
                    }
                });
        ...
        var DAction = function() {
            var text = $.trim($('#gmap_geocoding_address').val());
            var input = $('#LangLog_tmp').val();
            if(input=='') {
                GMaps.geocode({
                    address: text,
                    callback: function (results, status) {
        ...
        $('#edit_geocoding_btn').click(function(e) {
            e.preventDefault();
            DAction();
        });
