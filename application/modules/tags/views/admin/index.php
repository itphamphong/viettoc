<div class="col_full fleft">
    <form name="them" method="post" id="them" action=""	enctype="multipart/form-data">
        <div class="col-xs-6 list-btn fright">
            <a class="i-btn i-add"  href="<?php echo site_url("admin/tags/add/".$type)?>">Thêm mới</a>
                ...
                <?php  foreach ($list as $i) {
                    ...
                    <tr class="tr-product">
                        ...
                        <td>  <?php if($i->status==1){?>
                                <div class="checkbox-inline btn default btn-status status" data-id="<?php echo $i->id?>" data="0" onclick="Active(this)" data-url="<?php echo base_url("admin/tags/active") ?>">
                                    Đã kích hoạt
                                </div>
                            <?php }else{?>
                                <div class="checkbox-inline btn default btn-status no-status" data-id="<?php echo $i->id?>" data="1" onclick="Active(this)" data-url="<?php echo base_url("admin/tags/active") ?>">
                                    Chưa kích hoạt
                                </div>
                            <?php }?>
                        </td>
                        <td class=" ">
                            <a href="<?php echo base_url()?>admin/tags/edit/<?php echo $type."/". $i->id ?>" class="btn default btn-sm green ">
                                <i class="fa  icon-black"></i> Sửa
                            </a>
                            <a href="<?php echo base_url()?>admin/tags/delete/<?php echo $type."/".$i->id ?>"  class="btn default btn-sm red">
                                <i class="fa  icon-black "></i> Xoá
                            </a>
                        </td>
                    </tr>
                    <?php   foreach ($this->m_tags->show_list_tags_where(array("parent_id" => $i->id),1,1,"vn",0) as $j) {?>
                        <tr class="tr-product">
                            ...
                            <td>
                                <span title="Sửa" class="change_weight" onclick="ChangeW(this)"><?php echo $j->weight?></span>
                                <div class="col-center">
                                    <input type="text" name="weight[]" class="weight form-control">
                                </div>
                            </td>
                            <td></td>
                            <td class=" ">
                                <a href="<?php echo base_url()?>admin/tags/edit/<?php echo $type."/". $j->id ?>" class="btn default btn-sm green ">
                                    <i class="fa fa-pencil icon-black"></i> Sửa
                                </a>
                                <a href="<?php echo base_url()?>admin/tags/delete/<?php echo $type."/".$j->id ?>"  class="btn default btn-sm red">
