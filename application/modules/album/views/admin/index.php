<div class="col_full fleft">
    ...
    <form name="them" method="post" id="them" action=""	enctype="multipart/form-data">
        <div class="col-xs-6 list-btn fright">
            <a href="<?php echo base_url()?>admin/album/add/<?php echo $type ?>" class="btn default btn-sm green">
            ...
            <table class="table table-bordered">
                ...
                <?php  foreach ($list as $i) {
                    ...
                    <tr class="tr-product">
                        ...
                        <td>
                            <?php if($i->status==1){?>
                                <div class="checkbox-inline btn default btn-status status" data-id="<?php echo $i->id?>" data="0" onclick="Active(this)" data-url="<?php echo base_url("admin/album/active") ?>">
                                    <?php echo ACTIVE?>
                                </div>
                            <?php }else{?>
                                <div class="checkbox-inline btn default btn-status no-status" data-id="<?php echo $i->id?>" data="1" onclick="Active(this)" data-url="<?php echo base_url("admin/album/active") ?>">
                                    <?php echo NO_ACTIVE?>
                                </div>
                            <?php }?>
                        </td>
                        <td class=" ">
                            <a href="<?php echo base_url()?>admin/album/edit/<?php echo $type."/". $i->id ?>" class="btn  btn-sm i-btn">
                                <i class="fa fa-pencil icon-black"></i> Sửa
                            </a>
                            <a href="<?php echo base_url()?>admin/album/delete/<?php echo $type."/".$i->id ?>"  class="btn  btn-sm i-btn">
                                <i class="fa fa-times icon-black "></i> Xoá
                            </a>
                        </td>
                    </tr>
                    <?php   foreach ($this->m_album->show_list_album_where(array("parent_id" => $i->id),1,1,"vn",0) as $j) {?>
                        <tr class="tr-product">
                            ...
                            <td></td>
                            <td class=" ">
                                <a href="<?php echo base_url()?>admin/album/edit/<?php echo $type."/". $j->id ?>" class="btn  btn-sm i-btn">
                                    <i class="fa fa-pencil icon-black"></i> Sửa
                                </a>
                                <a href="<?php echo base_url()?>admin/album/delete/<?php echo $type."/".$j->id ?>"  class="btn  btn-sm i-btn">
                                    <i class="fa fa-times icon-black "></i> Xoá
                                </a>
                            </td>
                        </tr>
